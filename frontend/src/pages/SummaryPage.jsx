...
export default function SummaryPage() {
  ...
  const [allSessions] = useLocalStorage("poker_sessions", [])
  ...
  const sortedPlayers = [...players].sort((a, b) => ((b.cashOut || 0) - b.total) - ((a.cashOut || 0) - a.total))
  
  const biggestWinner = sortedPlayers.length > 0 && ((sortedPlayers[0].cashOut || 0) - sortedPlayers[0].total) > 0 ? sortedPlayers[0] : null;
  const biggestLoser = sortedPlayers.length > 0 && ((sortedPlayers[sortedPlayers.length - 1].cashOut || 0) - sortedPlayers[sortedPlayers.length - 1].total) < 0 ? sortedPlayers[sortedPlayers.length - 1] : null;

  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col items-center pt-8 sm:pt-16 px-4 pb-20">
      {/* Background Decor */}
      <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[50%] bg-primary/10 blur-[150px] rounded-full point-events-none" />
      <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-ring/10 blur-[150px] rounded-full point-events-none" />
      ...
      <div className="w-full max-w-4xl space-y-10 z-10 animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out fill-mode-both">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pb-6 border-b border-border/40">
          ...
          <Button variant="outline" className="w-full sm:w-auto rounded-full bg-background/50 backdrop-blur-md shadow-sm border-border/50" onClick={() => navigate(`/table/${session.tableId}`)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-6 duration-500 delay-100 fill-mode-both">
          <Card className="bg-primary/5 border-primary/20 shadow-sm backdrop-blur-sm relative overflow-hidden">
            ...
            <CardContent>
              <div className="text-4xl font-bold tracking-tight text-foreground">£{totalPot}</div>
              <p className="text-xs text-muted-foreground mt-2 font-medium flex items-center gap-1.5 break-all">
          ...
          <Card className={`${biggestWinner ? 'bg-amber-500/5 border-amber-500/20' : 'bg-card/40 border-border/30'} shadow-sm backdrop-blur-sm relative overflow-hidden`}>
            ...
            <CardContent>
              {biggestWinner ? (
                <>
                  <div className="text-2xl font-bold tracking-tight text-amber-500 drop-shadow-sm flex items-center gap-2 truncate">
                    {biggestWinner.name}
                  </div>
                  <Badge variant="outline" className="mt-2 bg-amber-500/10 text-amber-600 border-amber-500/20 px-2 py-0.5 text-xs font-bold">
                    +£{((biggestWinner.cashOut || 0) - biggestWinner.total).toFixed(2)}
                  </Badge>
                </>
              ) : (
          ...
          <Card className={`${biggestLoser ? 'bg-destructive/5 border-destructive/20' : 'bg-card/40 border-border/30'} shadow-sm backdrop-blur-sm relative overflow-hidden`}>
            ...
            <CardContent>
              {biggestLoser ? (
                <>
                  <div className="text-2xl font-bold tracking-tight text-destructive drop-shadow-sm flex items-center gap-2 truncate">
                    {biggestLoser.name}
                  </div>
                  <Badge variant="outline" className="mt-2 bg-destructive/10 text-destructive border-destructive/20 px-2 py-0.5 text-xs font-bold">
                    £{((biggestLoser.cashOut || 0) - biggestLoser.total).toFixed(2)}
                  </Badge>
                </>
              ) : (
